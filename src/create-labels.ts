...
function drawLabel(
  ...
  doc
    .fontSize(8)
    .text(
      'https://backtobasicskitchen.com/',
      x + LABEL_PADDING,
      y + LABEL_HEIGHT - LABEL_PADDING - 8,
      {
        width: LABEL_WIDTH - LABEL_PADDING * 2,
        align: 'center',
      }
...
export async function createLabels(orders: Orders = _orders): Promise<string> {
  return new Promise((resolve, reject) => {
    const menuItems = getOrderTotals(_orders);
    ...
    for (const title of Object.keys(menuItems)) {
      ...
      for (const variantTitle of Object.keys(variants)) {
        ...
        for (let i = quantity; i > 0; i--) {
          drawLabel(doc, getLabelRow(labelCount), getLabelColumn(labelCount), {
            title,
